...
unsigned int get_block_for_big_file(int file_node_num,int block_number_order, FILE* file_system){
    ...
    if (logical_block < 7){
        ...
        fseek(file_system, file_inode.addr[7]*block_size+(logical_block-7)*4, SEEK_SET);
...
void add_block_to_inode(int block_order_num, int block_num, int to_file_inode_num, FILE* file_system) {  
    inode_struct file_inode, tmp_node;
    int logical_block, prev_logical_block, logical_block_t, prev_logical_block_t, word_in_block, free_block_num, word_in_sec, second_block_num, word_in_third;
    ...
    if (word_in_block == 0 && logical_block <= 8){ 
        free_block_num = get_free_block(file_system);
    ...
    if (logical_block >= 7 && logical_block < 256) {
        ...
        if (logical_block > prev_logical_block){ 
            if (logical_block_t > prev_logical_block_t) { 
                ...
                fseek(file_system, file_inode.addr[8]*block_size+(logical_block_t)*4, SEEK_SET);
            ...
            fseek(file_system, triple_ind_block*block_size+(logical_block-8)*4, SEEK_SET);
            sec_ind_block = get_free_block(file_system);
            fwrite(&sec_ind_block,sizeof(sec_ind_block),1,file_system);
            fflush(file_system);
